...
unique_ptr<FunctionData> RetrieveDicomFuncBind(ClientContext &, TableFunctionBindInput &, vector<LogicalType> &,
                                               vector<string> &);
...
OFCondition subOpSCP(T_ASC_Association **, T_DIMSE_BlockingMode, unsigned int, Logger &, unsigned int &, bool,
                     Vector &);
OFCondition storeSCP(T_ASC_Association *, T_DIMSE_Message *, T_ASC_PresentationContextID, Logger &,
                     T_DIMSE_BlockingMode, unsigned int, unsigned int &, bool, Vector &);
